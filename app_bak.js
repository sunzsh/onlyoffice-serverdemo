  ...
  form.parse(req, (err, fields, files) => { // parse this form
    ...
    fileSystem.rename(file.path, `${uploadDir}/${file.name}`, (err) => { // rename a file
      ...
      if (err) { // if an error occurs
        ...
        const user = users.getUser(req.query.userid); // get user id and name parameters or set them to the default values
...
app.post('/convert', (req, res) => { // define a handler for converting files
  ...
  const callback = async function (err, res) {
    ...
    try {
      const responseData = documentService.getResponseUri(res.toString());
      ...
      const correctName = req.docManager.getCorrectName(fileUtility.getFileName(fileName, true) + newFileType); // get the file name with a new extension
      ...
      const correctHistoryPath = req.docManager.historyPath(correctName, userAddress, true); // get the history path to the file with a new extension
...
app.post('/reference', (req, res) => { // define a handler for renaming file
  ...
  if (!fileName && !!req.body.path) {
    const path = fileUtility.getFileName(req.body.path);
  ...
  const processTrack = async function (response, body, fileName, userAddress) {
    ...
    const callbackProcessSave = async function (downloadUri, body, fileName, userAddress, newFileName) {
      try {
        ...
        const versionPath = req.docManager.versionPath(newFileName, userAddress, version); // get the path to the specified file version
        ...
        if (downloadZip) {
          const path_changes = req.docManager.diffPath(newFileName, userAddress, version); // get the path to the file with document versions differences
        ...
        if (changeshistory) {
          const path_changes_json = req.docManager.changesPath(newFileName, userAddress, version); // get the path to the file with document changes
        ...
        const path_prev = path.join(versionPath, `prev${fileUtility.getFileExtension(fileName)}`); // get the path to the previous file version
        ...
        const forcesavePath = req.docManager.forcesavePath(newFileName, userAddress, false); // get the path to the forcesaved file
...
app.get('/editor-config', (req, res) => { // define a handler for editing document
  try {
    req.docManager = new docManager(req, res);
    ...
    if (fileExt != null) {
      ...
      const redirectPath = `${req.docManager.getServerUrl()}/editor?fileName=${encodeURIComponent(fileName)}${req.docManager.getCustomParams()}`;
    ...
    if (cfgSignatureEnable) {
      ...
      argss.editor.token = jwt.sign(argss, cfgSignatureSecret, { expiresIn: cfgSignatureSecretExpiresIn });
      ...
      res.json(argss); // render the editor template with the parameters specified
    } else {
      res.json(argss);
    }
  } catch (ex) {
    console.log(ex);
    res.status(500);
    res.json({ message: `Server error: ${ex.message}` });
